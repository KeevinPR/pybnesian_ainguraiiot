...
class BuildExt(build_ext):
    ...
    def create_clang_tidy_compilation_db(self, extensions):
        ...
        {{
            ...
            "file": "{1}",
            "output": "{2}",
            "arguments": ["/usr/lib/llvm-11/bin/clang", "-xc++", "{1}", "-Wno-unused-result", "-Wsign-compare", "-D", "NDEBUG", "-g", "-fwrapv", "-O0", "-Wall", "-g", "-fstack-protector-strong", "-Wformat", "-Werror=format-security", "-g", "-fwrapv", "-O0", "-g", "-fstack-protector-strong", "-Wformat", "-Werror=format-security", "-Wdate-time", "-D", "_FORTIFY_SOURCE=2", "-fPIC", "-D", "VERSION_INFO={3}", "-I", "{4}", "-I", "pybnesian/", "-I", "lib/libfort", "-I", "{5}", "-c", "-o", "{6}", "-std=c++17", "-isystem", "{6}", "-isystem", "{7}", "-isystem", "lib/eigen-3.3.7", "-isystem", "-isystem", "lib/boost", "-isystem", "lib/indicators", "-D", "-fdiagnostics-color=always", "-Wall", "-Wextra", "-fvisibility=hidden", "--target=x86_64-pc-linux-gnu"]
        }}"""
        conf_files = []
    ...
    def build_extensions(self):
        ...
        for ext in self.extensions:
            ...
            for source in ext.sources:
                ...
                obj_file = self.compiler.object_filenames([source], output_dir=self.build_temp)[0]
                obj_files.append(obj_file)
                
                # Only compile if the source or header is newer than the .so file
                if not os.path.exists(obj_file) or os.path.getmtime(source) > os.path.getmtime(obj_file) or os.path.getmtime(header) > os.path.getmtime(obj_file):
                    print(f"Recompiling due to changes in {source}")
                    
                    self.compiler.compile([source], output_dir=self.build_temp,
                                            macros=ext.define_macros,
                                            include_dirs=ext.include_dirs,
                                            extra_postargs=opts, depends=ext.depends)
                    
                    root_dir = './pybnesian/'
                    include_str = source[10:-3] + "hpp"  # header file, whether it exists or not
                    for dirpath, _, filenames in os.walk(root_dir):
                        for filename in filenames:
                            if filename.endswith(('.cpp')):
                                filepath = os.path.join(dirpath, filename)
                                with open(filepath, 'r', encoding='utf-8', errors='ignore') as file:
                                    if include_str in file.read(): # recompile dependencies
                                        self.compiler.compile([filepath], output_dir=self.build_temp,
                                            macros=ext.define_macros,
                                            include_dirs=ext.include_dirs,
                                            extra_postargs=opts, depends=ext.depends)
        ...
        # If any object files were recompiled, relink the shared object
        if recompile:
            os.remove(output_path)
            print(f"Linking objects for {ext.name} into {self.get_ext_fullpath(ext.name)}")
            self.compiler.link_shared_object(obj_files, self.get_ext_fullpath(ext.name),
                                                libraries=self.get_libraries(ext),
                                                library_dirs=ext.library_dirs,
                                                runtime_library_dirs=self.rpath,
                                                extra_postargs=link_opts, export_symbols=self.get_export_symbols(ext))
           
        else:
            # Skip recompiling if no sources are newer than the output
            print(f"Skipping recompilation, no changes detected.")
